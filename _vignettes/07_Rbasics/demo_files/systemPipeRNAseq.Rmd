---                                                                                                                                                                           
title: Rocket Science RNA-Seq Workflow Template 
...
output:                                                                                                                                                                       
  BiocStyle::html_document:                                                                                                                                                   
    ...
    code_folding: show                                                                                                                                                       
vignette: |                                                                                                                                                                   
  %\VignetteIndexEntry{Drug-Target Predictions Based on Signatures and Bioactivity Data}                                                                                      
...
Rscript -e "rmarkdown::render('systemPipeRNAseq.Rmd', c('BiocStyle::html_document', 'pdf_document')); knitr::knit('systemPipeRNAseq.Rmd', tangle=TRUE)"     
-->                                                                                                                                                                           
...
```{r source_helper_fcts, eval=TRUE}
...
```{r fastq_filter, eval=TRUE, message=FALSE, warning=FALSE}
...
suppressAll(preprocessReads(args=args, Fct="filterFct(fq, cutoff=20, Nexceptions=1)",
...
```{r fastq_report, eval=TRUE, message=FALSE, warning=FALSE}
args <- systemArgs(sysma="param/tophat.param", mytargets="targets.txt")
fqlist <- seeFastq(fastq=infile1(args)[1:2], batchsize=100000, klength=8)
png("./results/fastqReport.png", height=18, width=4*length(fqlist), units="in", res=300)
seeFastqPlot(fqlist)
dev.off()
``` 
...
genome sequence using `Bowtie2/HISAT2` [@Kim2013-vg; @Langmead2012-bs]. The parameter 
settings of the aligner are defined in the `hisat2.param` file.

```{r hisat_alignment2, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r bam_urls, eval=TRUE, message=FALSE, warning=FALSE}
symLink2bam(sysargs=args, htmldir=c("~/.html/", "gen242/today/"), 
...
```{r read_counting1, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r sample_tree, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r run_edger, eval=TRUE, message=FALSE, warning=FALSE}
library(edgeR); library(DT)
countDF <- read.delim("results/countDFeByg.xls", row.names=1, check.names=FALSE) 
targets <- read.delim("targets.txt", comment="#")
cmp <- readComp(file="targets.txt", format="matrix", delim="-")
edgeDF <- run_edgeR(countDF=countDF, targets=targets, cmp=cmp[[1]], independent=FALSE, mdsplot="")
edge_round <- na.omit(round(edgeDF, 4))
datatable(edge_round, filter = 'top', options = list(pageLength = 100, scrollX = TRUE, scrollY = "600px", autoWidth = TRUE))
```

Add gene descriptions

```{r custom_annot, eval=TRUE, message=FALSE, warning=FALSE}
library("biomaRt")
...
```{r filter_degs, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r venn_diagram, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r go_enrich, eval=TRUE, message=FALSE, warning=FALSE}
library("biomaRt")
...
DEGlist <- DEGlist[sapply(DEGlist, length) > 20]
...
```{r go_plot, eval=TRUE, message=FALSE, warning=FALSE}
gos <- BatchResultslim
png("./results/GOslimbarplotMF.png", height=9, width=8, units="in", res=300)
...
library(pheatmap); library(DESeq2, quietly=TRUE)
